(function(){
    ...
    Bridge.initRoot=function(){
        ...
        document.body.appendChild(Bridge.root.node);
        Bridge.root.width=innerWidth;
        Bridge.root.height=innerHeight;
        Bridge.root.node.style.overflow="hidden";
    }
    ...
    Bridge.initSprite=function(){
        ...
        proto.drawImage=function(url,baseTexW,baseTexH,texX,texY,texW,texH){
            ...
            if(url.match(/^\d*#/)){
                var a=url.split("#");
                var r=Math.min(parseInt(a[0]||0),width/2,height/2);
                var color=!a[1]?null:"#"+a[1];
                var bdColor=!a[2]?null:"#"+a[2];
                var bdSize=!bdColor?0:parseInt(a[3]||2);
                this.node.style.borderRadius=r+"px";
                this.node.style.background=color||"transparent";
                this.node.style.border=!bdColor?"":bdSize+"px solid "+bdColor;
                return;
            }
            if(texX==null) texX=0;
            if(texY==null) texY=0;
        ...
        proto.drawText=function(text,color,fontSize,width,align,spacing,leading){
            ...
            this.node.style.webkitUserSelect=this.selectabled?"text":"none";
            ...
            if(width==0){
                var div=Bridge.measureDiv;
                if(!div){
                    div=Bridge.measureDiv=document.createElement("div");
